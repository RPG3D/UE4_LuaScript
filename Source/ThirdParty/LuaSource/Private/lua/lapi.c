...
#define isvalid(L, o)	(!ttisnil(o) || o != &LUA_G(L)->nilvalue)
...
static TValue *index2value (lua_State *L, int idx) {
  ...
  if (idx > 0) {
    ...
    if (o >= L->top) return &LUA_G(L)->nilvalue;
    else return s2v(o);
  }
    ...
    if (ttislcf(s2v(ci->func)))  /* light C function? */
      return &LUA_G(L)->nilvalue;  /* it has no upvalues */
    else {
      CClosure *func = clCvalue(s2v(ci->func));
      return (idx <= func->nupvalues) ? &func->upvalue[idx-1] : &LUA_G(L)->nilvalue;
    }
  }
}
...
LUA_API void lua_xmove (lua_State *from, lua_State *to, int n) {
  ...
  api_check(from, LUA_G(from) == LUA_G(to), "moving among independent states");
...
LUA_API int lua_getglobal (lua_State *L, const char *name) {
  Table *reg = hvalue(&LUA_G(L)->l_registry);
  lua_lock(L);
