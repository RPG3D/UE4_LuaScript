...
int32 FastLuaUnrealWrapper::GenerateCodeForClass(const class UClass* InClass) const
{
	...
	//second, generate source
	{
		...
		for (TFieldIterator<const UProperty> It(InClass, EFieldIteratorFlags::ExcludeSuper); It; ++It)
		{
			...
			FString GetBodyStr = FString("\t") + FastLuaUnrealWrapper::GenerateGetPropertyStr(*It, PropName, InClass);
			...
			FString SetBodyStr = FString("\t") + FastLuaUnrealWrapper::GenerateSetPropertyStr(*It, PropName, InClass);
...
TArray<FString> FastLuaUnrealWrapper::CollectHeaderFilesReferencedByClass(const class UClass* InClass)
{
